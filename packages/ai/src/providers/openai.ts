...
export class OpenAIProvider implements LLMProvider {
  ...
  async chat(messages: Message[], options?: ChatOptions): Promise<ChatResponse> {
    ...
    const tools = options?.tools ? this.toOpenAITools(options.tools) : undefined;
    ...
    return this.retry.execute(async () => {
      try {
        ...
        const toolCalls = choice?.message?.tool_calls
          ? this.fromOpenAIToolCalls(choice.message.tool_calls as OpenAIToolCall[])
          : undefined;
    ...
    return messages.map((msg): OpenAIMessageParam => {
      ...
      switch (msg.role) {
        ...
        case "assistant": {
          ...
          return assistantMsg as unknown as OpenAIMessageParam;
        }
        case "tool":
          return {
            role: "tool",
  ...
  fromOpenAIToolCalls(toolCalls: OpenAIToolCall[]): ToolCall[] {
    return toolCalls
      .filter((tc): tc is OpenAIToolCall & { type: "function" } => tc.type === "function")
      .map((tc) => ({
        id: tc.id,
        name: tc.function.name,
        arguments: tc.function.arguments,
